...
const Minibag = (props) => {
  ...
  const [hideRemoveButton, setHideRemoveButtons] = useState(false)
  useEffect(() => {
    let total = 0;
  ...
  const showRemoveButton = ()=>{
    setHideRemoveButtons(!hideRemoveButton)
  }
  return (
    <>
      <div
        className={`sidebar ${props.isMinibagOpen && "sidebar-open"}`}
        ref={props.minibagRef}
      >
        <div className="sidebar-header">
          ...
          <img
            src={props.currentUser ? props.currentUser.photoURL : profilePic}
            className="userIcon"
          ></img>
          <span className="userName">
            {props.currentUser ? props.currentUser.displayName : "Login/Signup"}
          </span>
        </div>
          ...
          <>
            <div className="sidebar-order-list">
              <div className="list-title">
                <p>My order</p>
                <img src={editIcon} alt="" onClick={showRemoveButton} />
              </div>
              {props.cartItems.map((item) => {
                // setMinibagTotal(minibagTotal + item.cost)
                
                return <SidebarCartItem addToCart={props.addToCart} item={item} hideRemoveButton={hideRemoveButton} ></SidebarCartItem>;
              })}
            </div>
      ...
      {props.isMinibagOpen ? (
        <div className="sidebarBackdrop" onClick={props.closeMinibag}></div>
      ) : null}
    </>
  );
};
